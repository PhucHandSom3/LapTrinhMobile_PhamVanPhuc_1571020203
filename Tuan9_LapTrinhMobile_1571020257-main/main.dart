...
class User{
  ...
  String avatar;
  User({
    required this.id, required this.username, required this.password, required this.avatar
  });
}
      ...
      ListView.builder(
          ...
          itemBuilder: (context, index){
            return ListTile(
              ...
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (context) => UserDetail(users[index], cart)));
              },
            );
          }
      ) : Center(
...
class UserDetail extends StatelessWidget{
  final User user;
  final Cart cart;
  UserDetail(this.user, this.cart);
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          Image.network(user.avatar, width: 100, height: 100,),
          Padding(padding: EdgeInsets.all(0.0),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          itemBuilder: (context, index){
            return ListTile(
              title: Image.network(cart.items[index].avatar, width: 100, height: 100,),
              subtitle: Text(cart.items[index].username),
            );
          }
      )
    );
